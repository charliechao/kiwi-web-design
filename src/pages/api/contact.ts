...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  // Parse body
  let name: string, email: string, mobile: string,
  ...
  try {
    ...
    if (ct.includes('application/json')) {
      ...
      mobile   = (b.get('mobile')   as string) ?? '';
      business = (b.get('business') as string) ?? '';
      service  = (b.get('service')  as string) ?? '';
      message  = (b.get('message')  as string) ?? '';
    }
  } catch {
