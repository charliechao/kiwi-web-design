---
title: "How Much Does a Website Cost in New Zealand? (2026 Guide)"
description: "NZ website costs in 2026: small business sites $1,500–$6,000, eCommerce from $5,000, custom builds from $10,000. Real Auckland prices, what's included, and a live cost calculator."
pubDate: 2025-07-10
...
- Custom or complex builds: **$10,000–$50,000+** GST excl.
- Ongoing: hosting **$25–$120+/mo**, domain **~$25/yr**
- All quotes are **GST-exclusive** by default, add 15% for total cash outlay
...
| Starter small business | 3–5 pages | $1,000–$3,000 | New businesses, minimal content |
| Standard small business | 5–8 pages | $2,500–$6,000 | Service businesses, tradies |
| Small eCommerce | Up to 100 products | $5,000–$15,000+ | Retail, boutique stores |
...
<div id="kwd-calc" style="background:#f2f1e5;border-radius:12px;padding:1.75rem;margin:0 0 2rem">
  ...
  <div style="display:grid;gap:1rem;margin-bottom:1.25rem">
    <label style="display:flex;flex-direction:column;gap:0.35rem;font-size:0.875rem;font-weight:600;color:#2c2927">
      Site type
      <select id="kc-type" style="padding:0.6rem 0.75rem;border:1px solid rgba(0,0,0,0.15);border-radius:6px;font-size:0.95rem;background:#fff;color:#2c2927">
        <option value="800,2000">One-page / landing page</option>
        <option value="1000,3000">Starter small business (3–5 pages)</option>
        <option value="2500,6000" selected>Standard small business (5–8 pages)</option>
        <option value="5000,15000">Small eCommerce (up to 100 products)</option>
        <option value="10000,50000">Custom / complex build</option>
      </select>
    </label>
    <fieldset style="border:1px solid rgba(0,0,0,0.12);border-radius:8px;padding:0.875rem 1rem;background:#fff">
      <legend style="font-size:0.8rem;font-weight:700;text-transform:uppercase;letter-spacing:0.07em;padding:0 0.4rem;color:#2c2927">Add-ons</legend>
      <div style="display:grid;gap:0.5rem;margin-top:0.25rem">
        <label style="display:flex;align-items:center;gap:0.6rem;font-size:0.875rem;cursor:pointer;color:#2c2927"><input type="checkbox" id="kc-seo" value="400" style="accent-color:#c8a84b"> Basic on-page SEO setup (+$400)</label>
        <label style="display:flex;align-items:center;gap:0.6rem;font-size:0.875rem;cursor:pointer;color:#2c2927"><input type="checkbox" id="kc-booking" value="600" style="accent-color:#c8a84b"> Online bookings / appointments (+$600)</label>
        <label style="display:flex;align-items:center;gap:0.6rem;font-size:0.875rem;cursor:pointer;color:#2c2927"><input type="checkbox" id="kc-blog" value="300" style="accent-color:#c8a84b"> Blog setup (+$300)</label>
        <label style="display:flex;align-items:center;gap:0.6rem;font-size:0.875rem;cursor:pointer;color:#2c2927"><input type="checkbox" id="kc-integration" value="800" style="accent-color:#c8a84b"> Third-party integration, Xero, CRM (+$800)</label>
      </div>
    </fieldset>
    <label style="display:flex;flex-direction:column;gap:0.35rem;font-size:0.875rem;font-weight:600;color:#2c2927">
      Hosting / care plan
      <select id="kc-hosting" style="padding:0.6rem 0.75rem;border:1px solid rgba(0,0,0,0.15);border-radius:6px;font-size:0.95rem;background:#fff;color:#2c2927">
        <option value="0">None / bring your own</option>
        <option value="300">Basic ($25/mo), +$300/yr</option>
        <option value="708" selected>Business ($59/mo), +$708/yr</option>
        <option value="1188">Managed ($99/mo), +$1,188/yr</option>
      </select>
    </label>
    <label style="display:flex;align-items:center;gap:0.6rem;font-size:0.875rem;cursor:pointer;color:#2c2927"><input type="checkbox" id="kc-gst" style="accent-color:#c8a84b"> Include GST (15%)</label>
  </div>
  <div style="background:#2c2927;border-radius:8px;padding:1.25rem 1.5rem;display:flex;flex-wrap:wrap;justify-content:space-between;align-items:center;gap:1rem">
    <div>
      <div style="font-size:0.68rem;font-weight:700;text-transform:uppercase;letter-spacing:0.1em;color:rgba(255,255,255,0.45);margin-bottom:0.3rem">Estimated build cost</div>
...
(function () {
  ...
  function update() {
    var typeEl = document.getElementById('kc-type');
    var hostingEl = document.getElementById('kc-hosting');
    var gstEl = document.getElementById('kc-gst');
    var resultEl = document.getElementById('kc-result');
    var annualEl = document.getElementById('kc-annual');
    if (!typeEl || !resultEl) return;
    var range = typeEl.value.split(',').map(Number);
    var lo = range[0], hi = range[1];
    ['kc-seo', 'kc-booking', 'kc-blog', 'kc-integration'].forEach(function (id) {
      var el = document.getElementById(id);
      if (el && el.checked) { lo += Number(el.value); hi += Number(el.value); }
    });
    var hosting = Number(hostingEl.value);
    var gst = gstEl.checked ? 1.15 : 1;
    lo = Math.round(lo * gst);
    hi = Math.round(hi * gst);
    var hostingAmt = Math.round(hosting * gst);
    resultEl.textContent = fmt(lo) + ' \u2013 ' + fmt(hi);
    annualEl.textContent = hosting > 0 ? '+ ~' + fmt(hostingAmt) + '/yr ongoing' : 'No hosting / care plan selected';
  }
  ['kc-type', 'kc-hosting', 'kc-gst', 'kc-seo', 'kc-booking', 'kc-blog', 'kc-integration'].forEach(function (id) {
    var el = document.getElementById(id);
...
| Basic hosting | $25–$40/mo | Shared, essential updates |
| Business hosting + care | $50–$80/mo | Monitoring, backups, support |
| Managed care plan | $90–$120+/mo | Proactive optimisation, priority |
...
<div class="related-strip">
  ...
  <div class="rps-links">
    <a class="rps-link" href="/small-business-website-design-auckland/">Small Business Web Design</a>
    <a class="rps-link" href="/affordable-web-design-auckland/">Affordable Web Design Auckland</a>
    <a class="rps-link" href="/seo-auckland/">SEO Auckland</a>
    <a class="rps-link" href="/shopify-website-design-auckland/">Shopify Design NZ</a>
    <a class="rps-link" href="/woocommerce-website-design-auckland/">WooCommerce Design NZ</a>
  </div>
</div>
...
<script type="application/ld+json">
{
  ...
  "@graph": [
    {
      "@type": "WebPage",
      "@id": "https://www.kiwiwebdesign.co.nz/affordable-web-design-auckland/website-cost-new-zealand-small-business/",
      "speakable": {
    ...
    },
    {
      ...
      "mainEntity": [
        {
          "@type": "Question",
          "name": "What's the average cost of a 5-page website in NZ?",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "An Auckland 5-page small business website typically falls between $1,500 and $6,000 GST-exclusive. The range shifts based on copywriting needs, design rounds, and any extras such as booking forms or lead funnel setup."
          }
        },
        {
          "@type": "Question",
          "name": "How much does adding eCommerce cost in New Zealand?",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "Adding eCommerce to a standard brochure site generally adds $3,500–$9,000. Product setup, payment gateway configuration, shipping logic, and testing each require specialised time. Larger catalogues trend toward the upper end."
          }
        },
        {
          "@type": "Question",
          "name": "Do website prices include GST in New Zealand?",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "Project quotes are GST-exclusive by default in New Zealand. Adding GST increases the payable amount by 15%. A $3,000 website build would be $3,450 including GST."
          }
        },
        {
          "@type": "Question",
          "name": "What are the monthly and annual ongoing website costs in NZ?",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "Most Auckland businesses invest $300–$1,800+ per year on secure hosting, updates, backups, and support. Domain names renew at roughly $25 per year. Premium plugins or apps vary by platform and features required."
          }
        },
        {
          "@type": "Question",
          "name": "How long does a website build take in New Zealand?",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "A small business website typically takes 4–6 weeks from discovery to launch, assuming timely content delivery from the client. eCommerce or integration-heavy builds run 8–12 weeks to allow for testing and staff training."
          }
        },
        {
          "@type": "Question",
          "name": "Can I start with a small website and expand it later?",
          "acceptedAnswer": {
            "@type": "Answer",
            "text": "Yes. Many Auckland clients launch with a lean MVP and add booking tools, gated content, or product ranges once the site gains traction. Modular builds and care plans make iteration straightforward without re-platforming."
          }
        }
      ]
    }
